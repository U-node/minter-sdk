...
import hmac
import hashlib
from mnemonic.mnemonic import Mnemonic # BIP39 mnemonic
from two1.bitcoin.crypto import HDKey, HDPrivateKey, bitcoin_curve
...
class MinterWallet(object):
    ...
    def create(cls, mnemonic=None):
        ...
        master_key = HDPrivateKey(
                        key=int.from_bytes(binascii.unhexlify(IL), 'big'),
    ...
    def get_public_from_private(cls, private_key):
        ...
        public_key = bitcoin_curve.public_key(int.from_bytes(binascii.unhexlify(private_key), 'big'))
        
        return MinterPrefix.PUBLIC_KEY + binascii.hexlify(bytes(public_key)).decode()[2:]
